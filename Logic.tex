...
\begin{theorem}\label{connectives}
...
\begin{definition}
A formula in the form as $\theta$ in the proof above (\ref{connectives}) is said to be in \emph{disjunctive normal form} (dnf).
\end{definition}
...
Hence the dnf is:
\[ ((p_1 \land (\lnot p_2)) \lor ((\lnot p_1) \lor ( \lnot p_2))). \]
...
\begin{proof}
\begin{enumerate}
    \item By \ref{connectives} we just need to show, that $\land$ can be expressed using $\lnot,\lor$. $(p \land q)$ is l.e. to $(\lnot((\lnot p) \lor (\lnot q))$.
    \item similar to the approach above. $(p \lor q)$ is l.e. to $(\lnot((\lnot p) \land (\lnot q))$.
    \item Due to the cases above, it suffices to obtain either $\land$ or $\lor$ using $\lnot,\rightarrow$. $(p \lor q)$ is l.e. to $((\lnot p) \rightarrow  q)$.
\end{enumerate}
\end{proof}
